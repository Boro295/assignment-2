...
The authors of this paper are the members of Group 9.\\[0.8cm]

\newcommand{\sigentry}[3]{%
\begin{minipage}[t]{0.22\textwidth}
\centering
\textbf{#1}\\
#2\\[0.3cm]
\IfFileExists{#3}{\includegraphics[height=2.2cm]{#3}}{\underline{\hspace{2.8cm}}}\\[0.2cm]
\end{minipage}
}

\begin{center}
\sigentry{Adegunloye Fehintola Feyisayo}{210401008}{assets/sig1.png}
\hfill
\sigentry{Onabolu Samuel Ebunoluwa}{210401069}{assets/sig2.png}
\hfill
\sigentry{Oguejiofor-Abugu Belusochukwu}{210401037}{assets/sig3.png}
\hfill
\sigentry{Okpalaora Jeffry Chukwuebuka}{230401510}{assets/sig3.png}
\end{center}
...
\vspace{0.5cm}
\noindent\begin{tabular}{p{4.1cm}p{2.6cm}p{7.2cm}}
\toprule
\textbf{Member} & \textbf{Matric Number} & \textbf{Roles / Contributions} \\
\midrule
Adegunloye Fehintola Feyisayo & 210401008 & Programming/computation tasks; layout and typing of the report; writing of executive summary \\[0.4cm]
Onabolu Samuel Ebunoluwa & 210401069 & Programming/computation tasks; interpretation and discussion; leadership and team coordination \\[0.4cm]
Oguejiofor-Abugu Belusochukwu & 210401037 & Programming/computation tasks; conceptual formulation of solution; writing of conclusion \\[0.4cm]
Okpalaora Jeffry Chukwuebuka & 230401510 & Programming/computation tasks; results analysis and validation; proofreading and formatting \\
\bottomrule
\end{tabular}
...
Mathematical modelling and analysis is very important in Chemical Engineering. Engineers
must design specialised reactors and processes to create new substances or materials while
adhering to strict safety regulations and working to maximise efficiency from both an economic
and ecological standpoint. The aim of this paper was to establish a firm knowledge in analysing,
modelling and refining new and existing systems, establishing the balance between complexity
and accuracy and foster team-work between members towards creating or advancing industrial
processes. Four questions solved in four sections were evaluated using various numerical
methods and the computer application software SCILAB and Microsoft Excel. The solutions
were first computed manually, by making assumptions and applying theories. Then the
required formulas and lines of code were typed into the software. Problem 1 depicted a
chemical engineering process application involving 3 chemical reactors operating at steady
state. By making material balance around each reactor, a mass continuity system of three
simultaneous equations was generated. The equations were used to evaluate the concentrations
of the required specie in each reactor: $x_A$, $x_B$, and $x_C$ by solving the using Crammer's Rule,
Gaussian Elimination, and Matrix Inverse. The Matrix inverse method was used in solving the
system of equations on SCILAB and Microsoft Excel. The values of $x_A$, $x_B$, and $x_C$ were
gotten as 14.67\,mg/m$^3$, 7.33\,mg/m$^3$ and 10.10\,mg/m$^3$ respectively. Question 2 was in two
parts. Part A dealt with the determination of the temperature in a Resistance Temperature
Detector (RTD) operation. Using the Newton-Raphson and Interval Bisection methods, the
temperature of the detector when its resistance was 300$\Omega$ was calculated as 260.22$^\circ$C.
The Excel and SCILAB solutions also employed the Newton-Raphson equation to calculate
the required temperature. In part B, the volume of a real gas was determined at different
pressures by using the Van der Waals equation. The problem was solved in SCILAB with the
``fsolve'' function, and in Excel using ``Goal Seek''. Pressure was then plotted against volume.

The plot showed an inverse relationship between pressure and volume (Boyle's Law). A model
was derived for a liquid tank system with inlet and outlet volumetric flow rates in question 3.
The model related the volumetric inlet flow to the level of liquid in the tank. The model derived
was a lumped parameter model and the initial steady-state liquid level in the tank was
determined to be 25\,cm after making a key assumption. An expression for the dynamic response
of the liquid level to change in operation commands was also determined by linearising the
non-linear model equation. This was done using Taylor Series Expansion, and the resulting
ODE was evaluated with Laplace Transforms. The solution of the resulting equation was taken
to SCILAB to show the response of the system to different step lengths 0.01, 0.05, 0.1, 0.5, 1,
10 by using the Euler Explicit function and Runge-Kutta 4th order methods. In question 4, the
method of least-squares was used to best fit a function of the form $\ln(k) = C + b\ln(T) - D/T$
to the data given. This data showed the measurements of the rate coefficient, $k$ for a reaction at
different temperatures, $T$. By manually developing a system of three simultaneous equations
using least squares and Crammer's rule, the values $C$, $b$, and $D$ were obtained. The values
computed manually varied from the ones gotten in the software due to approximation and
possible random errors. From comparing the function and constants gotten to the log of the
Arrhenius equation, $k = AT^b e^{-E_a/RT}$, the values of $A$ and $E_a$ were deduced. The work
done is significant because it establishes the chemical engineering undergraduate in fundamental
principles of solving process problems.
...
Mathematical techniques and models are used in almost every element of a chemical engineer's
professional life. Modern industrial production is based on solid scientific methods, many of
which are part of chemical engineering. Engineers must design specialised reactors and
processes to create new substances or materials while adhering to strict safety regulations and
working to maximise efficiency from both an economic and ecological standpoint. Many
innovations in material design and process development are thought to be fuelled by
...
These techniques and models provide the mathematical foundations of different areas of
chemical engineering and describes typical applications. Modern chemical engineers are not
only expected to be adept at using these techniques; they are also expected to be able to apply
computer-based solutions when analytical ones are impractical.
...
This assignment was done as an assessment to test and establish the knowledge of Chemical
Engineering Analysis (CHG 433). Four questions given in four sections were evaluated using
various numerical methods and the computer application software SCILAB and Microsoft
Excel. The solutions were first computed manually, by making assumptions and applying
theories. Then the required formulas and lines of code were typed into the software mentioned
above.

Problem 1 depicted a chemical engineering process application involving 3 chemical reactors
operating at steady state. By making material balance around each reactor, a mass continuity
system of three simultaneous equations was generated. The equations were used to evaluate
the concentrations of the required specie in each reactor: $x_A$, $x_B$, and $x_C$ by solving the using
Crammer's Rule, Gaussian Elimination, and Matrix Inverse. The Matrix inverse method was
used in solving the system of equations on SCILAB and Microsoft Excel and the answers
obtained were all the same.

Question 2 was in two parts. Part A dealt with the determination of the temperature in a
Resistance Temperature Detector (RTD) operation. Using the Newton-Raphson and Interval
Bisection methods, the temperature of the detector when its resistance was 300$\Omega$ was
calculated as 260.22$^\circ$C. The Excel and SCILAB solutions also employed the Newton-Raphson
equation to calculate the required temperature. In part B, the volume of a real gas was
determined at different pressures by using the Van der Waals equation. The problem was solved
in SCILAB with the ``fsolve'' function, and in Excel using ``Goal Seek''. Pressure was then
plotted against volume and plotted in the Excel sheet. The plot showed an inverse relationship
between pressure and volume (Boyle's Law).

A model was derived for a liquid tank system with inlet and outlet volumetric flow rates in
question 3. The model related the volumetric inlet flow to the level of liquid in the tank. The
model derived was a lumped parameter model. An expression for the dynamic response of the
liquid level to change in operation commands was also determined by linearising the non-linear
model equation. This was done using Taylor Series Expansion, and the resulting ODE was
evaluated with Laplace Transforms. The solution of the resulting equation was taken to SCILAB
to show the response of the system to different step lengths by using the Euler Explicit function
and Runge-Kutta 4th order methods. The R-K graph is in reality more accurate than Euler's
method since it more accurately accounts for the spatial and other differences in the system.

In question 4, the method of least-squares was used to best fit a function of the form
$\ln(k)= C + b\ln(T) - D/T$ to the data given. This data showed the measurements of the rate
coefficient, $k$ for a reaction at different temperatures, $T$. By manually developing a system of
three simultaneous equations using least squares and Crammer's rule, the values of $C$, $b$,
and $D$ were obtained. The values computed manually varied from the ones gotten in the
software due to approximation and possible random errors. From comparing the function and
constants gotten to the log of the Arrhenius equation, $k = AT^b e^{-E_a/RT}$, the values of
$A$ and $E_a$ were deduced. All the problems were solved, typed, and formatted properly.
